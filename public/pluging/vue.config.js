...
module.exports = {
  ...
  publicPath: process.env.NODE_ENV === 'production' ? "/static/client/" : "./",// 部署应用时的根路径(默认'/'),也可用相对路径(存在使用限制)
  ...
  pages: {//pages 里配置的路径和文件名在你的文档目录必须存在 否则启动服务会报错
    index: {//除了 entry 之外都是可选的
      ...
      title: '华会',// 当使用 title 选项时,在 template 中使用：<title><%= htmlWebpackPlugin.options.title %></title>
  ...
  css: {
    ...
    loaderOptions: {
      sass: {
        prependData:`
        @import "src/assets/css/mycolor.scss";`
      }
    },// css预设器配置项
  ...
  devServer: {// 环境配置
    ...
    proxy: {// 配置多个代理(配置一个 proxy: 'http://localhost:4000' )
      '/api': {
        target: 'http://www.scottyue.com/qms',
